...
async def analyze_news_impact(title: str, summary: str) -> dict:
    ...
    if not OPENAI_API_KEY:
        ...
        # Return a helpful message when API key is not set
        return {
            "impacts": [], 
            "central_event": "", 
            "analysis_summary": "",
    ...
    try:
        prompt = f"""أنت محلل جيوسياسي واستراتيجي خبير متخصص في الشأن اليمني والعربي. مهمتك هي تحليل تأثير أي خبر عالمي على جميع الجوانب (وليس فقط الاقتصادية) مع التركيز على كيفية تأثر المواطن اليمني.

الخبر:
العنوان: {title}
الملخص: {summary}

حلل كيف سيؤثر هذا الخبر على جميع الجوانب:

## الجوانب المطلوب تحليلها:
1. **سياسياً**: العلاقات الدولية، التحالفات، القرارات السياسية
2. **أمنياً**: الاستقرار، الصراعات، التهديدات الأمنية
3. **اقتصادياً**: الأسواق، التجارة، الأسعار، العملات
4. **اجتماعياً**: الهجرة، اللاجئين، الحياة اليومية
5. **تقنياً**: التكنولوجيا، الإنترنت، الاتصالات
6. **بيئياً**: المناخ، الموارد الطبيعية، الكوارث
7. **إنسانياً**: المساعدات، الإغاثة، حقوق الإنسان
8. **عسكرياً**: التسليح، النزاعات، القواعد العسكرية
...
{{
    ...
    "impacts": [
        {{
            "id": "1",
            "sector": "اسم القطاع (أمثلة: السياسة الدولية، الأمن الإقليمي، الاقتصاد العالمي، الطاقة، التجارة، التكنولوجيا، الأمن الغذائي، الهجرة، المساعدات الإنسانية، الوضع العسكري، البيئة، الصحة العامة، الإعلام، الدبلوماسية)",
    ...
    "personal_impact": {{
        "daily_life": [
            {{
                "area": "المجال (مثل: أسعار الغذاء، الوقود، الكهرباء، الإنترنت، التحويلات المالية، الأمان، السفر، التأشيرات، الأدوية)",
                "effect": "التأثير المتوقع على حياتك اليومية في اليمن",
        ...
        "investments": [
            {{
                "type": "نوع الاستثمار (مثل: العقارات، الذهب، العملات، التجارة، المشاريع الصغيرة، الدولار)",
        ...
        "yemen_specific": {{
            ...
            "security_situation": "تأثير على الوضع الأمني في اليمن",
            "humanitarian": "تأثير على الوضع الإنساني والمساعدات",
...
قواعد مهمة:
- أعط من 5 إلى 8 تأثيرات متنوعة (ليست فقط اقتصادية)
- نوّع بين القطاعات: سياسي، أمني، اقتصادي، اجتماعي، إنساني
...
- فكر في التداعيات المتسلسلة: حدث عالمي -> تأثير إقليمي -> تأثير على اليمن -> تأثير على المواطن

مثال على سلسلة التداعيات:
إغلاق مضيق ملاحي -> ارتفاع تكاليف الشحن -> تأخر المساعدات الإنسانية لليمن -> نقص في المواد الغذائية -> ارتفاع الأسعار في الأسواق المحلية

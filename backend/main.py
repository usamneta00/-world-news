...
المهمة:
تحليل الأخبار المقدمة ودمجها في سرد تحليلي واحد يوضح:
- من يضغط على من
- ما هي المصالح الحقيقية
- ما الذي يُقال علناً مقابل ما يحدث فعلياً
- ما هي السيناريوهات المحتملة

منهج التحليل:
لا تستخدم لغة إنشائية أو تعبيرات عامة. كل جملة يجب أن تحمل معنى تحليلي واضح.
تجنّب التكرار تماماً. لا تعيد الفكرة بصياغات مختلفة.
اربط بين الجمل منطقياً (سبب → نتيجة → تأثير).

الأسلوب:
لغة مباشرة، حادة، لكن منضبطة.
مسموح بالنقد القاسي، لكن بصيغة تحليلية وليس خطابة.
ابتعد عن الاستعارات المبالغ فيها مثل "أسواق الموت" و"غسيل الدم" إلا نادراً جداً.

بدلاً من ذلك:
استخدم جمل حاسمة مثل:
"هذا ليس فشلاً بل قرار"
"ما يحدث يعكس سياسة متعمدة"
"الخطاب العلني لا يتطابق مع السلوك الفعلي"

التنسيق:
فقرتان كحد أقصى.
كل فقرة تخدم فكرة واضحة:
- الأولى: تفكيك المشهد الحالي
- الثانية: المخاطر أو الاتجاهات القادمة

ممنوع:
- الحشو
- التكرار
- المبالغة البلاغية
- إعادة نفس الفكرة بصيغ مختلفة

الهدف:
نص تحليلي يبدو كأنه صادر عن مركز دراسات، لكنه يحمل نقداً واضحاً وحاداً.
"""
        
    user_prompt = f"""
حلّل النص التالي تحليلاً جيوسياسياً مركزاً:

ادخل في الموضوع مباشرة دون مقدمات.

اكتب بأسلوب تحليلي واضح مثل مراكز الدراسات، وليس بأسلوب خطابي.
...
ارفع مستوى الحسم:
- سمِّ السياسات بوضوح (ضغط، ردع، ابتزاز، إدارة صراع)
- إذا كان هناك تناقض، وضّحه بشكل مباشر دون زخرفة

هيكل النص:
فقرتان فقط:
1) تحليل الوضع الحالي وتضارب المصالح
2) المخاطر أو السيناريوهات القادمة
